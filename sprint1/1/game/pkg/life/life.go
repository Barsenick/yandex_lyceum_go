...
func NewWorld(height, width int) (*World, error) {
	if height <= 0 {
		return &World{0, 0, make([][]bool, 0)}, errors.New("unpositive height value")
	}
	if width <= 0 {
		return &World{0, 0, make([][]bool, 0)}, errors.New("unpositive width value")
	}
	cells := make([][]bool, height)
	for i := range cells {
		cells[i] = make([]bool, width)
	}
	return &World{Width: width, Height: height, Cells: cells}, nil
}
